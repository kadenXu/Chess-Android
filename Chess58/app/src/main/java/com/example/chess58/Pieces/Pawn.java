...
import com.example.chess58.Pair;
...
public class Pawn extends Piece {
    ...
    /**
     * The pawn's piece taken by empassant
     */
    private Pair takenPieceLocationBySpecialMove;
    /**
     ...
     */
    public Pawn(Context context, String player, int color, boolean moveUp) {
        super(context, (color == Color.BLACK) ? R.drawable.pawn_black : R.drawable.pawn_white, player);
        this.player = player;
        this.name = "p";
        this.moveUp = moveUp;
    }
        ...
        if (diffRow == 1) {
            ...
            if (diffCol == 1 && cb.getPiece(new_row, new_col) == null) {
                ...
                if (conditionForEnPassant(curr_row, curr_col-1, cb.getPiece(curr_row, curr_col).getPlayer(), cb)) {
                    ...
                    takenPieceLocationBySpecialMove = new Pair(curr_row, curr_col - 1);
                    return true;
                }
    ...
    public boolean conditionForEnPassant (int row, int col, String currentPlayer, ChessBoard cb) {
        ...
        if (lastMovedPiece.row != row || lastMovedPiece.col != col) return false; // the captured pawn was not the most recently moved one
    ...
    /**
     * Set number of moves
     * @param moves number of moves to be set
     */
    public void setNumOfMoves(int moves){ moved = moves; }
     ...
     * @param value
     */
    public void setLastMoveWasFirstMove(boolean value){
        lastMoveWasFirstMove = value;
    }

    /**
     * Set the taken piece's location caused by empassant
     * @param pair
     */
    public void SetTakenPieceLocationBySpecialMove(Pair pair){
        takenPieceLocationBySpecialMove = pair;
    }
    /**
     * Get the take piece by empassant
     * @return
     */
    public Pair GetTakenPieceLocationBySpecialMove(){return takenPieceLocationBySpecialMove;}
}
